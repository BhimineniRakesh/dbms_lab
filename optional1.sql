...
--sum or avg)*/
/*ans)*/  SELECT name,rollno,(MAX( math+sci+eng+social+sports))/5 AS maxavg  FROM Student;
...
--second position of their name*/
 /*ans)*/ SELECT name FROM Student WHERE name LIKE '_a%';
...
--students in asc order of marks*/
 /*ans)*/ SELECT name ,math FROM Student ORDER BY math ASC;